...
#define MAX_PATH 128
#define SOME_FILE_NAME "key.dat"
#define SOME_DIR_NAME "user"
...
static bool create_some_entries(const char *base_path)
{
	...
	if (base >= (sizeof(path) - SOME_REQUIRED_LEN)) {
		printk("Not enough concatenation buffer to create file paths");
		return false;
	}

	printk("Creating some dir entries in %s", base_path);
	...
	if (fs_open(&file, path, FS_O_CREATE) != 0) {
		printk("Failed to create file %s", path);
		return false;
	}
	fs_close(&file);
	...
	if (fs_mkdir(path) != 0) {
		printk("Failed to create dir %s", path);
